	...
	useEffect(() => {
		...
		async function run() {
			...
			try {
				...
				// hidden 文章对未认证访客等同于不存在（真正的"下线"）
				if (blogData.config?.hidden && !(await hasAuth())) {
					if (!cancelled) {
						setError('文章不存在')
						setLoading(false)
					}
	...
	return (
		<>
			...
			<motion.button
				...
				whileTap={{ scale: 0.95 }}
				onClick={handleEdit}
				className='absolute top-4 right-6 rounded-xl border bg-white/60 dark:bg-white/10 px-6 py-2 text-sm backdrop-blur-sm transition-colors hover:bg-white/80 dark:hover:bg-white/15 dark:bg-white/10 max-sm:hidden'>
				编辑
			</motion.button>
